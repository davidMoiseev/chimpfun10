...
public class AutoRoutineRunner {
    ...
    public void update(){
        ...
        SmartDashboard.putNumber("auto drive step", driveStep);
        SmartDashboard.putNumber("ball drive step", ballStep);
        ...
        if(currentDriveMode == autoModes.followingSamplePath){
            ...
            SmartDashboard.putBoolean("autorunnerworking2", true);
            if(robotState.getTrajectoryComplete()){
                isDriveStepFinished = true;
                isPathFollowing = false;
            }
        ...
        if(currentBallMode == BallModes.shooting){
